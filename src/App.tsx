...
export default function Home() {
  ...
  const addTask = (task: { title: string; level: string; tag: string; time: string }) => {
    ...
    setTasks((current) => [...current, { id: Date.now(), ...task, coins: coinMap[task.level] || 5, done: false }]);
    setAddingTask(false);
  ...
  return (
    <main className="app-shell" onPointerDown={startEdgeSwipe} onPointerUp={endEdgeSwipe}>
      ...
      <header className={`topbar ${screen === "hoje" ? "today-topbar" : ""}`}>
        {screen !== "hoje" && <button className="brand" onClick={() => navigate("resumo")} aria-label="Ir para o resumo"><span>R</span>Rotina</button>}
      ...
      <section className="content" key={screen}>
        {screen === "resumo" && <Resumo greeting={greeting} done={done} total={tasks.length} workoutDone={workoutDone} workoutProgress={(series - 1) / 4 * 100} spiritualDone={spiritualDone} goalValues={goalValues} onGo={navigate} />}
        {screen === "hoje" && <Hoje tasks={tasks} toggle={toggleTask} remove={(id) => setTasks((current) => current.filter((task) => task.id !== id))} done={done} onAdd={() => setAddingTask(true)} />}
...
function Resumo({ greeting, done, total, workoutDone, workoutProgress, spiritualDone, goalValues, onGo }: { greeting: string; done: number; total: number; workoutDone: boolean; workoutProgress: number; spiritualDone: boolean; goalValues: number[]; onGo: (s: Screen) => void }) {
  ...
  useEffect(() => {
    ...
    if (dayPercent >= 70) {
      ...
      return () => { window.clearTimeout(start); window.clearTimeout(celebrate); if(finish) window.clearTimeout(finish); };
    }
  ...
  return <>
    ...
    <div className="section-title next-title"><h2>Próximos</h2><button onClick={() => onGo("hoje")}>Ver dia →</button></div>
    <div className="next-grid">
      <button className="next-mini" onClick={() => onGo("hoje")}><i className="green-line"/><b>Personal</b><small>17:00</small></button>
      <button className="next-mini" onClick={() => onGo("hoje")}><i className="blue-line"/><b>Muay Thai</b><small>18:30</small></button>
    </div>
...
function Hoje({ tasks, toggle, remove, done, onAdd }: { tasks: typeof tasksSeed; toggle: (n: number) => void; remove: (n:number)=>void; done: number; onAdd:()=>void }) {
  ...
  useEffect(()=>{
    ...
    if(crossed){ setCelebrating(true); const timer=window.setTimeout(()=>setCelebrating(false),6200); return ()=>window.clearTimeout(timer); }
  },[percent]);
